...
import { AppDocument, ProcessNode, ProcessEdge, Project, ChecklistItem, CAPAReport } from '../../types';
import { useTranslation } from '../../hooks/useTranslation';
// FIX: Import the correct and newly added icons for the process map editor.
import { TrashIcon, ShareIcon, PlayIcon, CogIcon, XMarkIcon, FolderIcon, ExclamationTriangleIcon, StopCircleIcon, DiamondIcon } from '../icons';
...
const ProcessNodeDetailPanel: FC<{ document: AppDocument, projects: Project[] }> = ({ document, projects }) => {
        ...
        for (const project of projects) {
            for (const item of project.checklist) {
                ...
                if (item.evidenceFiles.includes(document.id)) {
                    ...
                    const capas = project.capaReports.filter(c => c.sourceChecklistItemId === item.id);
                    capaReports.push(...capas.map(c => ({...c, projectName: project.name})));
                }
            }
        }
    ...
    return (
        <div className="absolute top-4 right-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border dark:border-dark-brand-border rounded-lg shadow-lg w-80 max-h-[calc(100%-2rem)] flex flex-col pointer-events-auto animate-[fadeIn_0.3s_ease-out]">
            <h4 className="text-sm font-semibold p-3 border-b dark:border-dark-brand-border">{t('linkedItems')}</h4>
            <div className="p-3 overflow-y-auto space-y-4">
                <div>
                    <h5 className="text-xs font-bold uppercase text-gray-500 mb-2 flex items-center gap-1.5"><FolderIcon className="w-4 h-4" />{t('linkedChecklistItems')}</h5>
                    {linkedItems.checklistItems.length > 0 ? (
                        <ul className="space-y-2">
                           {linkedItems.checklistItems.map(item => <li key={item.id} className="text-xs p-2 bg-slate-100 dark:bg-slate-900/50 rounded-md"><strong className="text-brand-primary">{item.standardId}</strong> in {item.projectName}</li>)}
                        </ul>
                    ) : <p className="text-xs text-center text-gray-400 py-2">{t('noLinkedItems')}</p>}
                </div>
                 <div>
                    <h5 className="text-xs font-bold uppercase text-gray-500 mb-2 flex items-center gap-1.5"><ExclamationTriangleIcon className="w-4 h-4"/>{t('linkedCapaReports')}</h5>
                    {linkedItems.capaReports.length > 0 ? (
                        <ul className="space-y-2">
                            {linkedItems.capaReports.map(capa => <li key={capa.id} className="text-xs p-2 bg-red-50 dark:bg-red-900/20 rounded-md"><strong className="text-red-500">{capa.id}</strong> from {capa.projectName}</li>)}
                        </ul>
                    ) : <p className="text-xs text-center text-gray-400 py-2">{t('noLinkedItems')}</p>}
                </div>
            </div>
                ...
                <div className="flex flex-grow overflow-hidden">
                    ...
                    <main className="flex-grow relative dot-grid cursor-grab active:cursor-grabbing" onWheel={handleWheel} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
                        ...
                        <div className="absolute top-0 left-0 pointer-events-none" style={{ transform: `translate(${viewTransform.x}px, ${viewTransform.y}px) scale(${viewTransform.scale})`, transformOrigin: '0 0' }}>
                         ...
                         {detailNode && <ProcessNodeDetailPanel document={document} projects={projects} />}
                    </main>
                </div>
            </div>
        </div>
