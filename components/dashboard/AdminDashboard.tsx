  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg">
        ...
        <p className="text-sm text-brand-primary dark:text-brand-primary-400">{`${t(
          "complianceRate"
        )}: ${payload[0].value}%`}</p>
      </div>
    );
...
const AdminDashboard: React.FC<DashboardPageProps> = ({ setNavigation }) => {
  ...
  const dashboardData = useMemo(() => {
    ...
    const openCapaReports = projects.flatMap((p) =>
      p.capaReports.filter((c) => c.status === "Open")
    );

    const thirtyDaysFromNow = new Date();
    thirtyDaysFromNow.setDate(thirtyDaysFromNow.getDate() + 30);
    const upcomingProjectDeadlines = projects.filter(
      (p) =>
        p.endDate &&
        new Date(p.endDate) <= thirtyDaysFromNow &&
        new Date(p.endDate) > new Date()
    );
    const upcomingDocReviews = documents.filter(
      (d) =>
        d.reviewDate &&
        new Date(d.reviewDate) <= thirtyDaysFromNow &&
        new Date(d.reviewDate) > new Date()
    );
    const upcomingDeadlinesCount =
      upcomingProjectDeadlines.length + upcomingDocReviews.length;
          ...
          {dashboardData.recentOpenCapa.length > 0 ? (
            dashboardData.recentOpenCapa.map((capa: CAPAReport) => {
              ...
              return (
                <div
                  key={capa.id}
                  className="p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-brand-border dark:border-dark-brand-border"
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div>
                      <p className="font-semibold text-brand-text-primary dark:text-dark-brand-text-primary">
                        {capa.sourceStandardId || t("incidentSource")}
                      </p>
                    ...
                    <div className="flex items-center gap-4 text-sm mt-2 sm:mt-0">
                      ...
                      >
                        {t(capa.type.toLowerCase() as any)}
                      </span>
                      <span>
