...
const DepartmentDetailPage: React.FC<DepartmentDetailPageProps> = ({ department, users, projects, currentUser, setNavigation, onUpdateDepartment, onDeleteDepartment }) => {
    ...
    const stats = useMemo(() => {
        const applicableTasks = departmentTasks.filter(t => t.status !== ComplianceStatus.NotApplicable);
        ...
        const complianceRate = applicableTasks.length > 0 ? Math.round((score / applicableTasks.length) * 100) : 0;
        return {
        ...
        <>
            <div className="space-y-6">
                <div className="bg-brand-surface dark:bg-dark-brand-surface p-6 rounded-lg shadow-sm border border-gray-200 dark:border-dark-brand-border">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                        <div className="flex items-center space-x-4 rtl:space-x-reverse">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-brand-text-primary dark:text-dark-brand-text-primary">{department.name[lang]}</h1>
                                <p className="text-brand-text-secondary dark:text-dark-brand-text-secondary">{t('departmentDetails')}</p>
                            </div>
                        </div>
                        {canModify && (
                            <div className="flex items-center gap-2">
                                 <button onClick={() => setIsEditModalOpen(true)} className="p-2 text-gray-500 hover:text-brand-primary"><PencilIcon className="w-5 h-5" /></button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard title={t('assignedUsers')} value={stats.userCount} icon={UsersIcon} color="bg-emerald-600"/>
                    <StatCard title={t('tasksAssigned')} value={stats.taskCount} icon={ClipboardDocumentCheckIcon} color="bg-indigo-600"/>
                    <StatCard title={t('complianceRate')} value={stats.complianceRate} color="bg-blue-600"/>
                </div>
