...
interface DepartmentsPageProps {
  ...
  onCreateDepartment: (dept: Omit<Department, 'id'>) => void;
  onUpdateDepartment: (dept: Department) => void;
  onDeleteDepartment: (deptId: string) => void;
}

const DepartmentsPage: React.FC<DepartmentsPageProps> = ({ departments, users, projects, currentUser, setNavigation, onCreateDepartment, onUpdateDepartment, onDeleteDepartment }) => {
  ...
  const departmentStats = useMemo(() => {
    ...
    return departments.map(dept => {
        ...
        const tasksForDept = allChecklistItems.filter(item => item.assignedTo && userIdsInDept.has(item.assignedTo));
        const applicableTasks = tasksForDept.filter(c => c.status !== ComplianceStatus.NotApplicable);
        ...
        const compliance = applicableTasks.length > 0 ? (score / applicableTasks.length) * 100 : 100;
  ...
  const totalTasks = useMemo(() => projects.flatMap(p => p.checklist).length, [projects]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title={t('totalDepartments')} value={departments.length} icon={BuildingOffice2Icon} color="bg-blue-600"/>
        <StatCard title={t('totalStaffAssigned')} value={totalStaff} icon={UsersIcon} color="bg-emerald-600"/>
        <StatCard title={t('totalTasks')} value={totalTasks} icon={ClipboardDocumentCheckIcon} color="bg-indigo-600"/>
      </div>
