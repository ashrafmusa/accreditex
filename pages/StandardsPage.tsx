...
const StandardsPage: React.FC<StandardsPageProps> = ({
  ...
  const { addStandard, updateStandard, deleteStandard } = useAppStore();
  ...
  const handleOpenEditModal = (standard: Standard) => {
  ...
  const filteredStandards = useMemo(() => {
    return standards.filter((s) => {
      const subStandardsText =
        s.subStandards
          ?.map((sub) => `${sub.id} ${sub.description}`)
          .join(" ") || "";
      const searchContent = `${s.standardId} ${s.description} ${
        s.section || ""
      } ${subStandardsText}`.toLowerCase();
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div className="flex items-center space-x-3 rtl:space-x-reverse self-start">
          ...
          <div>
            <h1 className="text-3xl font-bold dark:text-dark-brand-text-primary">
              {program.name} {t("standardsLibrary")}
            </h1>
        ...
        {canModify && (
          <button
            ...
            className="bg-brand-primary text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center font-semibold shadow-sm w-full md:w-auto"
          >
      ...
      <div className="bg-brand-surface dark:bg-dark-brand-surface p-4 rounded-lg shadow-sm border border-gray-200 dark:border-dark-brand-border flex flex-wrap items-center gap-4">
        <div className="relative flex-grow min-w-[200px]">
          ...
          <input
            ...
            className="w-full ltr:pl-10 rtl:pr-10 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-brand-primary focus:border-brand-primary bg-white dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
          />
        </div>
      </div>
      ...
      {Object.keys(groupedStandards).length > 0 ? (
        <div className="space-y-8">
          ...
          {Object.entries(groupedStandards).map(
            ([section, standardsInSection]) => (
              <div key={section}>
                <h2 className="text-xl font-semibold text-brand-text-primary dark:text-dark-brand-text-primary mb-4 pb-2 border-b-2 border-brand-primary dark:border-indigo-400">
                  {section}
                </h2>
